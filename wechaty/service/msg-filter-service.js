...
async function filterFriendMsg(msg, name, id) {
    ...
    if (msg === '开启聊天模式') {
        const list = await api.getUserInfo(name);
        if (list.length) {
            await api.updateUserInfo({setter: name, autoReply: true});
        } else {
            await api.setUserInfo({setter: name, autoReply: true});
        }
    } else if (msg === '关闭聊天模式') {
        const list = await api.getUserInfo(name);
        if (list.length) {
            await api.updateUserInfo({setter: name, autoReply: false});
        } else {
            await api.setUserInfo({setter: name, autoReply: false});
        }
    }
    const list = await api.getUserInfo(name);
    if (list.length) {
